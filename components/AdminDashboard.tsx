...
import { getNextWeekMonday, getWhatsAppLink, ALL_CLASSES, ALL_SECTIONS, OFFLINE_SUBMISSIONS_KEY, SUBMISSION_RETRY_KEY, getCurrentWeekMonday } from '../constants';
...
const AdminDashboard: React.FC<Props> = ({ 
  ...
  const handleManualCompile = async (className: string, section: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCompileStatus(`❌ Failed to compile PDF`);
      }
    } catch (error) {
  ...
  const handleSendAllReminders = async () => {
    ...
    if (!confirm(`Send reminders to ${defaulters.length} defaulters?`)) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        alert(`Reminders sent to ${defaulters.length} defaulters`);
      } else {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden">
        <div className="flex border-b border-gray-50 bg-gray-50/50">
           {['monitor', 'registry', 'requests', 'compile', 'settings'].map(t => (
                ...
                <div className="flex justify-between items-center">
                  ...
                  {missingTeachers.length > 0 && (
                    <button 
                      ...
                      className="bg-blue-600 text-white px-6 py-3 rounded-xl font-black uppercase text-xs hover:bg-blue-700"
                    >
                    ...
                    {(Object.entries(defaultersByClass) as [string, Teacher[]][]).map(([cls, list]) => (
                      <div key={cls} className="bg-gray-50 p-6 rounded-[2.5rem] border border-gray-100">
                        <div className="flex justify-between items-center mb-4">
                          ...
                          <span className="text-xs font-bold text-gray-500">{list.length} pending</span>
                        </div>
          ...
          {activeTab === 'compile' && (
            <div className="space-y-8">
              <div className="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100">
                <h3 className="text-2xl font-black text-gray-800 mb-4">Manual PDF Compilation</h3>
                <p className="text-gray-600 mb-6">
                  Generate and send compiled lesson plans to class teachers. 
                  Automatic compilation runs every Saturday at 8 PM.
                </p>
                
                {compileStatus && (
                  <div className={`p-4 rounded-xl mb-6 ${compileStatus.includes('✅') ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                    {compileStatus}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {allClasses.map(cls => {
                    ...
                    return (
                      <div key={cls} className="bg-white p-6 rounded-2xl border border-gray-100">
                        <h4 className="font-black text-gray-900 text-lg mb-2">Class {cls}</h4>
                        <button
                          onClick={() => handleManualCompile(className, section)}
                          disabled={isCompiling}
                          className="w-full bg-purple-600 text-white py-3 rounded-xl font-black uppercase text-xs hover:bg-purple-700 disabled:opacity-50"
                        >
                          {isCompiling ? 'Compiling...' : 'Generate & Send PDF'}
                        </button>
                      </div>
                    );
                  })}
          ...
          {activeTab === 'settings' && (
            <div className="space-y-8">
               <div className="bg-gray-50 p-8 rounded-[2rem] border border-gray-100">
                  ...
                  <input 
                    type="text" 
                    className="w-full px-6 py-4 rounded-xl bg-white border font-bold text-sm" 
                    value={syncUrl} 
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    ...
                    This enables email notifications, PDF compilation, and cloud sync.
                  </p>
               </div>
               <div className="flex flex-col gap-4">
                 ...
                 <button onClick={() => onResetRegistry?.()} className="text-left text-red-500 font-black uppercase text-xs tracking-widest p-6 hover:bg-red-50 rounded-2xl transition-colors">
                   <i className="fas fa-database mr-2"></i> Factory Reset Local Registry
                 </button>
               </div>
            </div>
          )}
